...
async function banter() {
  ...
  const prompt = 'You write the two commentators of STONK WARS, a live bracket where 16 contestants (animals with brains scaled to neuron counts, or famous meme characters with brains built from their personalities) trade memecoins for ' + Math.round((config.STONK_MATCH_MS || 900000) / 60000) + ' minutes per match. The two commentators are the SAME man — the Stonks meme guy in the suit — sitting side by side at a desk, and they argue.\n' +
...
async function interview() {
  ...
  if (key) {
    ...
    const prompt = 'You are ' + a.name.toUpperCase() + ', a contestant in STONK WARS — trading memecoins (' + a.statText + '). Character: ' + a.blurb + ' Quirk: ' + a.brain.quirk + '. Memory: can hold ' + a.brain.memory + ' coins in mind. Reaction time: ' + (a.brain.reactionMs / 1000) + 's. Patience: ' + a.brain.maxHoldMin + ' minutes.\n' +
...
async function chatBanter() {
  ...
  if (key) {
    ...
    const prompt = 'You write the two commentators of STONK WARS, a live bracket where 16 contestants (animals with brains scaled to neuron counts, or famous meme characters with brains built from their personalities) trade meme coins for ' + Math.round((config.STONK_MATCH_MS || 900000) / 60000) + ' minutes per match. They are the SAME man — the Stonks meme guy in the suit — at a desk. STONKS MAN (left): the bull, calm, certain, meme cadence ("Stonks." "Line go up."). NOT STONKS MAN (right): the identical man, the bear ("Not stonks." "Line go down."), dry doom.\n' +
